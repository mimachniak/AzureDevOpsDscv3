...
<#
.SYNOPSIS
    DSC v3 resource script for managing Entra group membership in Azure DevOps project-level groups.

.DESCRIPTION
    Implements Get, Set, and Test operations for an Entra security group's membership
    in a project-level Azure DevOps security group (e.g. Contributors, Readers).
    ...
    'Get' {
        try {
            ...
            if ($null -ne $sourceGroupDesc) {
                $isMember = Test-IsMember -Organization $org -SubjectDescriptor $sourceGroupDesc -ContainerDescriptor $projGroupDesc -Token $token -ApiVersion $apiVersion
            }
    ...
    'Set' {
        try {
            ...
            if ($ensure -eq 'Present') {
                ...
                Write-DscTrace -Level Info -Message "Adding Entra group '$originId' to '$groupDisplayName' in project '$projectName'."
                $encSource = [uri]::EscapeDataString($sourceGroupDesc)
                $encTarget = [uri]::EscapeDataString($projGroupDesc)
                ...
                if ($null -ne $sourceGroupDesc) {
                    Write-DscTrace -Level Info -Message "Removing Entra group '$originId' from '$groupDisplayName' in project '$projectName'."
                    $encSource = [uri]::EscapeDataString($sourceGroupDesc)
                    $encTarget = [uri]::EscapeDataString($projGroupDesc)
                    $uri = "https://vssps.dev.azure.com/$org/_apis/graph/memberships/$($encSource)/$($encTarget)?api-version=$apiVersion"
