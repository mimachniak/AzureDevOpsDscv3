...
<#
.SYNOPSIS
    DSC v3 resource script for managing user membership in Azure DevOps project-level groups.

.DESCRIPTION
    Implements Get, Set, and Test operations for user membership in a project-level
    Azure DevOps security group (e.g. Contributors, Readers, Project Administrators).
    Requires network access to https://dev.azure.com and https://vssps.dev.azure.com,
    and a PAT with 'Graph > Read & manage' and 'Project and Team > Read' permissions.
...
# Map the permissionLevel value to the Azure DevOps built-in group display name
...
function Get-ProjectId {
    param([string]$Organization, [string]$ProjectName, [string]$Token)
...
function Get-ProjectScopeDescriptor {
    param([string]$Organization, [string]$ProjectId, [string]$Token, [string]$ApiVersion)

    $uri      = "https://vssps.dev.azure.com/$Organization/_apis/graph/descriptors/$($ProjectId)?api-version=$ApiVersion"
...
function Get-ProjectGroupDescriptor {
    ...
    if ($null -eq $groups -or $null -eq $groups.value) {
        throw "No groups found for the specified project scope."
    }
    $group = $groups.value | Where-Object { $_.displayName -eq $GroupDisplayName } | Select-Object -First 1
    if ($null -eq $group) {
        throw "Group '$GroupDisplayName' not found in the project."
    }
...
switch ($Operation) {
    ...
    'Get' {
        try {
            ...
            $groupDescriptor = Get-ProjectGroupDescriptor -Organization $org -ScopeDescriptor $scopeDescriptor -GroupDisplayName $groupDisplayName -Token $token -ApiVersion $apiVersion
    ...
    'Set' {
        try {
            ...
            if ($ensure -eq 'Present') {
                Write-DscTrace -Level Info -Message "Adding '$upn' to '$groupDisplayName' in project '$projectName'."
                ...
                Write-DscTrace -Level Info -Message "Removing '$upn' from '$groupDisplayName' in project '$projectName'."
                $uri = "https://vssps.dev.azure.com/$org/_apis/graph/memberships/$($encUser)/$($encGroup)?api-version=$apiVersion"
